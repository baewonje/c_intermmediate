...
void main(){
		...
		// const의 이름은 모두 대문자로 쓰는 것이 관레(naming convention)이다.
		const unsigned long long MY_PASSWORD = 1234567890123456789;
		unsigned long long input_password = 0;
		const int MY_MAX;
		// MY_MAX의 값을 최초로 정의(define) 하지만 그 이전에 MY_MAX가 선언
		// 되는 과정에서 초기화 되었으므로 아래 라인은 const 값의 변경을
		// 시도하는 것이기 때문에 컴파일 에러를 발생한다.
